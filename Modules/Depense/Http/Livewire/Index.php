...
class Index extends Component
{
    ...
    public $action, $selectedItem,$action2;
    ...
    public $type_compteID,$sous_type_compteID,$DepensesFiltre,$fournisseur,$date_debut;
    public $entreprise, $is_default, $solde, $prenom, $telephone, $email, $titre, $adresse, $ville, $pays, $paiements_modalite_id, $devises_monetaire_id, $paiements_mode_id;
    public $showModalArticle = false;
    public $showModalClient = false;
    public $showModalCompte = false;
    public $showModalPaiement=false;
    public $nom_produit,$nom_service,$nom_stock,$comptes_comptable_default,$prix_achat,$qte_stock,$comptes_achat_id,$prix_unitaire_stock,$qte_initiale,$comptes_vente_id,$date_debut_stock;
    public $according = 0,$affiche='',$case='disabled',$cocher="Désactiver";
    ...
    public function openAddDepenseModal($action)
    {
        ...
        if ($action == 'facture') {
            ...
            $this->dispatchBrowserEvent('openPaiementModal');
         }
    ...
    public function closeExpressPaiementModal()
    {
        if ($this->action == "paiement_add") {
    ...
    public function closeExpressModal($type)
    {
        if ($type == "article") {
            ...
            $this->closeExpressPaiementModal();
            $this->showModalPaiement = false;
        }
    }

    private function resetInputPaiementFields()
    {
        $this->entreprise_id = "";
        $this->type = "";
        $this->nom = "";
    ...
    private function resetInputFields()
    {
        ...
        $this->nom_stock = null;
        $this->comptes_comptable_default = null;
        $this->prix_achat = null;
        $this->qte_stock = null;
        $this->comptes_achat_id = null;
        $this->comptes_vente_id = null;
        $this->prix_unitaire_stock = null;
        $this->qte_initiale = 1;
        $this->date_debut_stock = null;
        $this->according = 0;
        $this->affiche = "";
        $this->case = "";
        $this->cocher = "";
    ...
    public function saveProduit_HS(Request $request)
    {
        ...
        $this->validate($validateData);
        $articlesData = new DepenseController;
        $articlesData->addNewProduit_HS($request->all()['serverMemo']['data']);
        $this->resetInputFields();
        $this->closeExpressModal('article');
    }
    ...
    public function saveService(Request $request)
    {
        ...
        $this->validate($validateData);
        $articlesData = new DepenseController;
        $articlesData->addNewService($request->all()['serverMemo']['data']);
        $this->resetInputFields();
        $this->closeExpressModal('article');
    }
    ...
    public function saveStock(Request $request)
    {
        ...
        $this->validate($validateData);
        $articlesData = new DepenseController;
        $articlesData->addNewStock($request->all()['serverMemo']['data']);
        $this->resetInputFields();
        $this->closeExpressModal('article');
    }
    ...
    public function saveComptePaiement(Request $request)
    {
        $validateData = [
        ...
        $this->validate($validateData);
        $comptePaiementData = new DepenseController;
        $comptePaiementData->addNewComptePaiement($request->all()['serverMemo']['data']);
        $this->resetInputPaiementFields();
        $this->closeExpressModal('paiement');
    }

    public function realTimeFetchComptePaiement()
    {
        if ($this->action == "paiement_add") {
            $this->dispatchBrowserEvent('openDepenseAddModal');
        }
    ...
    public function realAfficheAccording()
    {
      if ($this->according == 0) {
        $this->affiche="hide";
        $this->case="disabled";
        $this->cocher="Désactiver";
    ...
    public function updated()
    {
        ...
        $this->realTimeFetchDepense();
          //type compte
        $this->realType_compteID();
        // type compte paiement
        $this->realTimeFetchComptePaiement();
        // type shox according or hide
        $this->realAfficheAccording();
    }
