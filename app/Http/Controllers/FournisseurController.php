...
class FournisseurController extends Controller
{
    ...
    public function details($id)
    {
        if (Auth::user()->role == "cabinet" && SessionControl::where('entreprise_id', Auth::user()->entreprise_id)->where('status', "valide")->first()) {
            ...
            $Depenses= DB::table('depenses')->where('fournisseur_id',  $id)->get();
            $total_activite=DB::table('depenses')->where('fournisseur_id', $id)->where('type','<>', 'credit')->sum('total');
            $total_credit=DB::table('depenses')->where('fournisseur_id', $id)->where('type', 'credit')->sum('total');

            $PageName = "Modification infos fourniseur";
            $DevisesMonetaires = DevisesMonetaire::all();
            $ComptesComptables = Comptescomptable::where('entreprise_id', $EntrepriseID)->get()();
            $ModesPaiements = PaiementsMode::where('entreprise_id', $EntrepriseID)->get()();
            return view('entreprise.depenses.fournisseurs.details', compact('total_credit','total_activite','Fournisseur' , 'PageName', 'DevisesMonetaires', 'ComptesComptables', 'ModesPaiements', 'Session','Depenses'));
        } else {
            $Session = 0;
            $EntrepriseID = Auth::user()->entreprise_id;
