<script>
    ...
    function calculate(elementID) {
        ...
        $("#amount_commercial_facture_pay").html(formatNumber.format(MTTC));
    ...
    $(document).on("click", ".trash", function (e) {
        ...
        if(rowCount > 1){
            ...
            if(!isNaN(TTC) && !isNaN(MHT) && !isNaN(Taxe)){
                ...
                $("#amount_commercial_facture_pay").html(formatNumber.format(TTC));
            }
...
<script>
    $('#numero_facture').keyup(function () {
      $('#get_facture_number_commercial').text($(this).val());
    });

    // Recupération automatique du date d'échéance

    $('#date_facturation_facture').change(function(e){
        e.preventDefault();
        ...
        var date = document.getElementById('date_facturation_facture').value;
        var echeance = new Date(date);
        var net = document.getElementById('paiements_modalite_id_facture').value;
        ...
        $('#echeance_facture').val(date_echeance);
    });

    $('#paiements_modalite_id_facture').change(function(e){
        e.preventDefault();
        Date.prototype.addDays = function (days) {
            const date = new Date(this.valueOf());
            date.setDate(date.getDate() + days);
