    ...
    <div class="card-header">
        <div class="row flex-between-center">
            <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">{{$PageName}}</h5> <br>
    ...
    <div class="card-body bg-light">
        <div class="tab-content">
            <div class="tab-pane preview-tab-pane active" role="tabpanel"
                ...
                <form action="{{route('entreprise.employe.update', $Employe->id)}}" class="row g-3 col-md-12 needs-validation" novalidate="" method="POST" enctype="multipart/form-data"> @csrf
                    <div class="col-md-6">
                        <label class="form-label" for="prenom">{{__('messages.first_name')}}</label>
                        <input class="form-control" name="prenom" id="prenom" value="{{$Employe->prenom}}" type="text" required="" />
                    </div>

                    <div class="col-md-6">
                        <label class="form-label" for="nom">{{__('messages.last_name')}}</label>
                        <input class="form-control" name="nom" id="nom" value="{{$Employe->nom}}"  type="text" required="" />
                    </div>

                    <div class="col-md-6">
                        <label class="form-label" for="email">{{__('messages.email')}}</label>
                        <input class="form-control" id="email" name="email" value="{{$Employe->email}}"  type="text" required="" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="telephone">{{__('messages.telephone')}}</label>
                        <input class="form-control" name="telephone" id="telephone" value="{{$Employe->telephone}}"  type="text" required="" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="genre">{{__('messages.gender')}}</label>
                        <select class="form-select" name="genre" id="genre" required="">
                            <option disabled="" value="">{{__('messages.chose')}}</option>
                            <option value="homme" @if ($Employe->interval_paiement == "homme") selected @endif>{{__('messages.male')}}</option>
                            <option value="femme" @if ($Employe->interval_paiement == "femme") selected @endif>{{__('messages.female')}}</option>

                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="date_naissance">{{__('messages.date_of_birth')}}</label>
                        <input class="form-control" name="date_naissance" value="{{$Employe->date_naissance}}"  id="date_naissance" type="text" required="" />
                    </div>

                    <div class="col-md-12">
                        <label class="form-label" for="adresse">{{__('messages.address')}}</label>
                        <input class="form-control" name="adresse" id="adresse" value="{{$Employe->adresse}}"  type="text" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="ville">{{__('messages.town')}}</label>
                        <input class="form-control" id="ville" name="ville" value="{{$Employe->ville}}"  type="text" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="province">{{__('messages.province')}}</label>
                        <input class="form-control" name="province" id="province" value="{{$Employe->province}}"  type="text" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="code_postale">{{__('messages.postal_code')}}</label>
                        <input class="form-control" name="code_postale" id="code_postale" value="{{$Employe->code_postale}}"  type="text" />
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input class="form-control" name="poste" id="poste" type="text" value="{{$Employe->poste}}"  required="" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="date_embauche">{{__('messages.hiring_date')}}</label>
                        <input class="form-control" name="date_embauche" id="date_embauche" value="{{$Employe->date_embauche}}"  type="date" required="" />
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="remuneration">{{__('messages.remuneration')}}</label>
                        <input class="form-control" name="remuneration" id="remuneration" value="{{$Employe->remuneration}}"  type="number" required="" />
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="interval_paiement"   id="interval_paiement" required="">
                            <option value="1" @if ($Employe->interval_paiement == 1) selected @endif>{{__('messages.day')}}</option>
                            <option value="7" @if ($Employe->interval_paiement == 7) selected @endif>{{__('messages.week')}}</option>
                            <option selected="" value="30" @if ($Employe->interval_paiement == 30) selected @endif>{{__('messages.month')}}</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="paiements_mode_id"   id="paiements_mode_id" required="">
                            ...
                            <option value="{{$pmode->id}}"  @if ($Employe->paiements_mode_id == $pmode->id) selected @endif>{{$pmode->nom}}</option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">{{__('messages.you_must_field_this_case')}}</div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="devises_monetaire_id" id="devises_monetaire_id" required="">
                            ...
                            <option value="{{$devise->id}}"  @if ($Employe->devises_monetaire_id == $devise->id) selected @endif>{{$devise->nom}} {{$devise->sigle}}</option>
                            @endforeach
                    ...
                    <div class="col-12">
                        <button class="btn btn-success w-100" type="submit">{{__('messages.update')}}</button>
                    </div>
                </form>
            </div>
