...
<div class="card-body">
  ...
  <tr>
  <td>{{$depense->type}}</td>
  <td>{{$depense->facture_id??'-- '}}</td>
  <td>{{$depense->note??'-- '}}</td>
  @if($depense->type=='credit')<td class="text-danger">-{{$depense->total}}</td>@else<td >{{$depense->total}}@endif
  @if($depense->status=='validated')<td class="text-success">Payée</td>
  @else<td  class="text-danger">Impayée <button type="button" class="btn btn-success">Payer</button></td>@endif
</tr>
@endforeach
</tbody>
</table>
    ...
    <div class="col-lg-4 ps-lg-2">
      <div class="sticky-sidebar">
        <div class="card mb-3">
          ...
          <div class="card-body">
          <p>TOTAL SANS CREDIT AJOUTE: {{$total_activite}}</p>  
          <p>TOTAL CREDIT: -{{$total_credit}}</p>  
          <p>TOTAL AVEC CREDIT AJOUTE: {{$total_activite-$total_credit}}</p> 
          <p><button type="button" class="btn btn-success">Payer</button></p> 
          </div>
        </div>
