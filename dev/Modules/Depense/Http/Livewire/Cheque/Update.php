...
namespace Modules\Depense\Http\Livewire\Cheque;
...
{
    public $fournisseur, $date_paiement, $numero_cheque, $note, $fichier, $paiements_modalite_id;
    ...
    public function getChequeData($id, $action = null)
    {
        $Depense = getDepenseById($id);
        $this->DepenseData = $Depense;
        $this->taxes = getDepenseDataBySession()['Taxes'];
        //dd($Depense);
        if ($Depense->cheque->clients_entreprise_id > 0) {
            $this->fournisseur = "C".$Depense->cheque->clients_entreprise_id ?? '';
        }elseif ($Depense->cheque->fournisseur_id > 0) {
            $this->fournisseur = "F".$Depense->cheque->fournisseur_id ?? '';
        }

        $this->IdDepense = $Depense->id;
        $this->date_paiement = $Depense->cheque->date_paiement;
        $this->numero_cheque = $Depense->cheque->numero_cheque;
        $this->note = $Depense->note;
        $this->paiementsource_id = $Depense->paiementsource_id;
        $this->paiements_mode_id = $Depense->cheque->paiements_mode_id;
    ...
    {
        if ($nom == 'compte') {
            $this->emit('openExpressWithUpdate', $this->IdDepense, 'cheque', 'compte', 'update');
        }elseif ($nom == 'client') {
            $this->emit('openExpressWithUpdate', $this->IdDepense, 'cheque', 'client', 'update');
        }elseif ($nom == 'article') {
            $this->emit('openExpressWithUpdate', $this->IdDepense, 'cheque', 'article', 'update');
        }
    }
    ...
    {
        $this->dispatchBrowserEvent('swal:confirm-delete-compte-cheque',[
            'type' => 'warning',
    ...
    {
        $this->dispatchBrowserEvent('swal:confirm-delete-article-cheque',[
            'type' => 'warning',
    ...
    public function deleteCompteCheque($articleId)
    {
        (new DepensesPanierController)->destroy($articleId);
        $this->emit('getChequeData', $this->IdDepense);
    }

    public function confirmDeleteFacture()
    {

        $this->dispatchBrowserEvent('swal:confirm-delete-cheque',[
            'type' => 'warning',
            'title' => 'Voulez-vous vraiment supprimer ce chèque ?',
            'text' => '',
            'id' => $this->IdDepense,
            'typeFacture' => 'cheque'
        ]);
    ...
    public function update(Request $request)
    {
        ...
        $this->dispatchBrowserEvent('swal:modal',[
            'type' => 'success',
            'title' => 'Chèque Modifié avec succès!',
            'text' => ''
        ]);
    ...
    public function render()
    {
        ...
        return view('depense::livewire.cheque.update', compact('data'));
    }
}
