                      ...
                      <div class=" mb-3">
                        ...
                        <div class="card-body">
                           <form action="{{route('admin.parametres.packages.modules.update', $Package->id)}}" class="row g-3 needs-validation" novalidate="" method="POST" enctype="multipart/form-data"> @csrf
                            ...
                            <div class="ml-5 col-md-4 form-check form-switch">
                                <input class="form-check-input  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 19)->first()) bg-success @endif" value="1" name="utilisateurs" id="fffdd" type="checkbox"  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 19)->first()) checked @endif />
                                <label class="form-check-label" for="fffdd">{{$Modules->where('id', 19)->first()->nom}}</label>
                            </div>
                            <div class="ml-5 col-md-4 form-check form-switch">
                                <input class="form-check-input  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 20)->first()) bg-success @endif" value="1" name="abonnements" id="fffdd" type="checkbox"  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 19)->first()) checked @endif />
                                <label class="form-check-label" for="fffdd">{{$Modules->where('id', 20)->first()->nom}}</label>
                            </div>
                            <div class="ml-5 col-md-4 form-check form-switch">
                                <input class="form-check-input  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 21)->first()) bg-success @endif" value="1" name="infos_entreprise" id="fffdd" type="checkbox"  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 21)->first()) checked @endif />
                                <label class="form-check-label" for="fffdd">{{$Modules->where('id', 21)->first()->nom}}</label>
                            </div>
                            <div class="ml-5 col-md-4 form-check form-switch">
                                <input class="form-check-input  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 22)->first()) bg-success @endif" value="1" name="habilitations" id="fffdd" type="checkbox"  @if ($ModulePack->where('package_id', $Package->id)->where('module_id', 22)->first()) checked @endif />
