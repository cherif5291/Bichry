  ...
  <div class="row g-3 mb-3">
    <div class="col-lg-12">
      <div class="card h-100">
        ...
        <form action="{{route('entreprise.facture.changeFactureStatus', $Facture->id)}}" method="POST">@csrf
            <div class="row col-md-12 d-flex justify-content-between gx-3 mb-3">
                <div class="col-md-5">
                  ...
                  <select class="form-select mb-3" name="fournisseur" id="fournisseur" aria-label="Default select example">
                    ...
                    <option  @if ($Facture->fournisseur_id === $client->id) selected @endif  value="{{$client->id}}">{{$client->entreprise}}</option>
                    @endforeach
                  </select>
                </div>
                <div class="col-md-4">
              ...
              <div class="row gx-3 col-md-12 d-flex justify-content-between mb-3">
                <div class="col-md-5">
                  ...
                  <input class="form-control" name="adresse_facturation" id="adresse_facturation" type="text" value=" @if ($Facture) {{$Facture->adresse_facturation}} @endif" />

                </div>
                <div class="col-md-4">
                    <label class="form-label ls text-uppercase text-600 fw-semi-bold mb-0 fs--1" for="paiements_mode_id">Mode de paiement</label>
                    <select class="form-select mb-3" name="paiements_mode_id" id="paiements_mode_id" aria-label="Default select example">
                      <option >choisir</option>
                      @foreach ($ModePaiements as $ModalP)
                      <option @if ($Facture)@if ($Facture->paiements_mode_id === $ModalP->id) selected @endif @endif value="{{$ModalP->id}}">{{$ModalP->nom}}</option>
                      @endforeach
                    </select>
                </div>

                <div class="col-md-2">
                    <label class="form-label ls text-uppercase text-600 fw-semi-bold mb-0 fs--1" for="numero_recu">Nº de recu de vente</label>
                    <input class="form-control" id="numero_recu" name="numero_recu" type="text" value=" @if ($Facture) {{$Facture->numero_recu}} @endif" />
                </div>
              ...
              <div class="row gx-3 col-md-12  mb-3">
                ...
                <div class="col-md-4">
                    <label class="form-label ls text-uppercase text-600 fw-semi-bold mb-0 fs--1" for="date_recu">Date de recu de vente </label>
                    <input class="form-control" name="date_recu" id="date_recu" value=" @if ($Facture){{$Facture->date_recu}}  @endif"   type="date" />
                </div>


                <div class="col-md-4">
                    <label class="form-label ls text-uppercase text-600 fw-semi-bold mb-0 fs--1" for="reference">Nº de reference</label>
                    <input class="form-control" id="reference" name="reference" type="text" value=" @if ($Facture) {{$Facture->reference}} @endif" />
                </div>

                <div class="col-md-4">
                    <label class="form-label ls text-uppercase text-600 fw-semi-bold mb-0 fs--1" for="paiement_source_id">Déposer sur</label>
                    <select class="form-select mb-3" name="paiement_source_id" id="paiement_source_id" aria-label="Default select example">
                      @foreach ($SourcesPaiement as $source)
                      <option @if ($Facture)@if ($Facture->paiement_source_id === $source->id) selected @endif @endif value="{{$source->id}}">({{$source->type}}) {{$source->nom}}</option>
                      @endforeach
                    </select>
                </div>
              ...
              <div class="row gx-3 col-md-12 d-flex justify-content-between mb-3">
                ...
                <div class="col-md-4">
                    <label class="form-label ls text-uppercase text-600 fw-semi-bold mb-0 fs--1" for="message">Message sur la facture</label>
                    <textarea name="message" id="" class="form-control" cols="30" id="message" rows="4"> @if ($Facture) {{$Facture->message}} @endif</textarea>
                </div>
                <div class="col-md-4">
                    ...
                    <textarea name="message_affiche" id="" class="form-control" cols="30" id="message_affiche" rows="4"> @if ($Facture) {{$Facture->message_affiche}} @endif</textarea>
                </div>
                <div class="col-md-4">
                   <div>
                  ...
                  <tbody>
                      ...
                      <tr>
                        ...
                        <td class="align-middle text-end">
                            <a href="{{route('admin.factures.panier.delete', $article->id)}}">
                            ...
                            <div class="modal " id="error-modal" tabindex="-1" role="dialog" aria-hidden="true">
                                ...
                                <div class="modal-content position-relative" >
                                    ...
                                    <div class="rounded-top-lg py-3 ps-4 pe-6 bg-light">
                                        <h4 class="mb-1" id="modalExampleDemoLabel">Ajouter un produit ou service à facturer</h4>
                                    </div>
                                    <div class="p-4 pb-0">
                                        <form action="{{route('entreprise.facture.addArticletoDepense')}}" method="POST" class="col-md-12 row"> @csrf
                                               <div class="col-md-12">
                                               ...
                                               <div class="mt-3 col-md-6">
                                                ...
                                                <div class="mt-3 col-md-6">
                                                    <input class="form-control" hidden name="facture_id" type="text" min="1" value="{{$Facture->id}}" />
                ...
                <div class="col-auto">
                  <table class="table table-sm table-borderless fs--1 text-end">
                    <tr>
                      ...
                      <td class="fw-semi-bold">{{$DepensePaniers->where('facture_id', $Facture->id)->sum('montant')}} </td>
                    </tr>
                    <tr>
                      <th class="text-900">Taxe {{$DepensePaniers->where('facture_id', $Facture->id)->sum('taux_taxe')??0/$DepensePaniers->where('facture_id', $Facture->id)->count()}}%:</th>
                      <td class="fw-semi-bold">{{$DepensePaniers->where('facture_id', $Facture->id)->sum('montant_taxe')??0}} Fcfa*</td>
                    </tr>

                    <tr class="border-top border-top-2 fw-bolder text-900">
                      <th>SOLDE À PAYER :</th>
                      <td>{{$DepensePaniers->where('facture_id', $Facture->id)->sum('total')}} FCFA*</td>
                    </tr>
                  </table>
                </div>
              </div>
